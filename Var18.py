...
def calculate_survival_rates(data, max_age=None):
    ...
    survival_rate = {
        embarked: (survival_count[embarked] / total_count[embarked]) * 100 if total_count[embarked] > 0 else 0
...
def do_var18():
    st.title('Анализ выживаемости на Титанике')
    max_age = st.slider(
    ...
    prepared_data = [{'Пункт посадки': embarked, 'Доля выживших': rate} for embarked, rate in survival_rate_age.items() if rate != 100.00]
